...
当历史对话超过 Token 阈值时，可以让模型把较早内容压缩成摘要，同时保留最近几条原始消息：
...
调用模型生成摘要
    ↓
保存：SystemMessage(历史摘要) + 最近 4 条消息
```

本项目的 `/ai/memory/summary` 使用 800 tokens 作为演示阈值。页面中选择“摘要记忆（摘要 + 最近4条）”即可体验。

摘要记忆会额外产生一次模型调用，因此生产环境需要考虑调用成本、超时、失败重试和摘要质量校验。本示例先关注核心流程。

## 10. 异步摘要与流式输出

同步摘要会阻塞 `ChatMemory.add()`，从而影响 SSE 流式响应。本项目新增 `/ai/memory/summary-async`：
